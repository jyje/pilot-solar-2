# Case 04 — LangChain OpenWiki로 Solar Open2를 이용해 pilot-solar-2 문서화
...
리포 자체**(`pilot-solar-2`)를 대상으로 최신 커밋을 문서화하고 질문에
...
[`scripts/verify.sh`](scripts/verify.sh)는 `pilot-solar-2`를 git-ignore된
...
헤더)만으로 생성하며, Case 01·02의 Python 도구들과 달리 `authToken`(→
`Authorization: Bearer`)은 전혀 지원하지 않습니다. 직접 확인했습니다:
raw `@anthropic-ai/sdk` JS 클라이언트로 `apiKey`만 사용해 Upstage의
Anthropic 호환 엔드포인트를 호출하니 **즉시 401 `invalid_api_key`**가
돌아왔습니다(행이 아니라 명확한 거부 — Upstage의 Anthropic 호환
엔드포인트가 `x-api-key` 인증을 그냥 거부합니다). 현재 코드로는 확실한
막다른 길입니다.
...
이건 `openwiki`나 `deepagents` 자체 코드 문제가 아니라 실제 Upstage/Solar
Open2 스트리밍 버그(또는 클라이언트-서버 청킹 불일치)입니다. 하지만
`openwiki`는 이 provider 경로에서 스트리밍을 끌 방법을 제공하지 않았기
때문에, **포크에 작은 패치를 추가**했습니다(`jyje/openwiki`, 브랜치
`fix/disable-streaming-for-tool-calling-providers`): 새 
`OPENWIKI_DISABLE_STREAMING=true` 환경변수가 범용 provider 분기의
`ChatOpenAI`에 `streaming: false`를 설정하도록 — 옵트인 방식이라 다른
모든 `openai-compatible` 계열 provider는 기존처럼 스트리밍을 유지합니다.
확인 결과: 이 플래그를 설정하면 동일하게 실패하던 요청이 이제 정확한
툴 이름과 함께 성공합니다.
...
트래픽과 무관하게 단일 실행만으로도 Upstage 기본 **분당 50,000
토큰** rate limit을 초과하기 충분합니다. 이건 코드 버그가 아니라
용량/티어 제약입니다. `scripts/verify.sh`는 여전히 이를 시도(best-effort)
하지만 통과 여부를 게이팅하지 않습니다 — 아래 3가지 질문(저렴하고
단일 턴인 호출)이 안정적으로 통과하는 핵심 체크입니다.
...
근거 실행: *(이 케이스의 첫 CI 실행 이후 추가 예정 — [검증](#검증) 참고)*
...
best-effort). [`jyje/openwiki`](https://github.com/jyje/openwiki/tree/fix/disable-streaming-for-tool-calling-providers)의
